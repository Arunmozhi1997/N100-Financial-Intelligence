...
def check_data_types(conn, table):
    ...
    for column in df.columns:
        ...
        if df[column].dtype not in ["int64", "float64", "object"]:
...
def validation_summary():
    ...
    for table in tables:
        ...
        nulls = check_null_values(conn, table)

        duplicates = check_duplicates(conn, table)

        pk = check_primary_key(conn, table, primary_keys[table])

        if table != "companies":

            check_foreign_keys(conn, table)

        missing = check_required_columns(conn, table, required_columns[table])

        datatype = check_data_types(conn, table)

        year_issue = check_year_format(conn, table)

        negative_issue = check_negative_values(conn, table)

        percentage_issue = check_percentage_range(conn, table)

        empty_issue = check_empty_table(conn, table)

        print("Year Issues:", year_issue)

        print("Negative Values:", negative_issue)

        print("Percentage Issues:", percentage_issue)

        print("Empty Table:", empty_issue)
